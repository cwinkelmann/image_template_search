...
def _get_points_and_labels(
    img_path: str, df: pd.DataFrame
    ...
    points = [
        (r["x"] / w, r["y"] / h) for r in records
    ]  # normalise the point coordinates
    labels = [r["labels"] for r in records]  # get the labels

    return points, labels


def _get_points_and_labels_hA(
    hA_image: AnnotatedImage,
) -> tuple[List[tuple[float, float]], List[str]]:
    w, h = hA_image.width, hA_image.height

    points = [
        (int(r.centroid.x) / w, int(r.centroid.y) / h) for r in hA_image.labels
    ]  # normalise the point coordinates
...
def _create_keypoints(
    points: list[tuple[int, int]], labels: list[str]
) -> List[fo.Keypoint]:
    keypoints = []
...
def _create_boxes(
    boxes: typing.List[shapely.Polygon], labels: typing.List[str]
) -> typing.List[fo.Detection]:
    """
...
def _create_boxes_s(hA_image: AnnotatedImage) -> typing.List[fo.Detection]:
    ...
    for r in hA_image.labels:
        ...
        kp = fo.Detection(
            label=str(lab),
            bounding_box=[x1, y1, box_w, box_h],
            # kind="str",
            hasty_id=r.id,
            # attributes=r.attributes,
            # attributes={"custom_attribute": [{"bla": "keks"}]},
...
def debug_hasty_fiftyone(
    ...
    for image_path in images_set:
        ...
        assert (
            len(hA_gt_sample) == 1
        ), f"There should be one single image left, but {len(hA_gt_sample)} are left."
...
def debug_hasty_fiftyone_v2(
    ...
    if annotated_images is not None:
        ...
        assert all(
            isinstance(img, AnnotatedImage) for img in annotated_images
        ), "Each item should be an AnnotatedImage object"
...
def debug_hasty_fiftyone_v3(
    image_path: Path, df_labels: pd.DataFrame, dataset: fo.Dataset
):
    """
    Display these annotations in Fifty One
    :return:
    ...
    def get_keypoints(df: pd.DataFrame, kind):
        ...
        for i, r in df[df.kind == kind].iterrows():
            ...
            pt = (r["x"] / 5472, r["y"] / 3648)  # TODO get this size from the image
            lab = r["label"]
    ...
    sample["True Positives"] = fo.Keypoints(
        keypoints=get_keypoints(df=df_labels, kind="true_positive")
    )
    sample["False Positives"] = fo.Keypoints(
        keypoints=get_keypoints(df=df_labels, kind="false_positive")
    )
    sample["False Negatives"] = fo.Keypoints(
        keypoints=get_keypoints(df=df_labels, kind="false_negative")
    )
...
if __name__ == "__main__":
    ...
    images_dir = (
        base_path / "San_STJB01_10012023/template_images/San_STJB01_10012023_DJI_0068"
    )
    drone_image = images_dir / "San_STJB01_10012023_DJI_0068.JPG"
    image_2 = (
        base_path / "San_STJB01_10012023/San_STJB01_10012023_orthomosaic_DDeploy.tif"
    )

    annotations_file_path = (
        base_path / "Snt_STJB01_10012023/template_images/methods_paper_labels.json"
    )
    ...
    projected_annotation_path = Path(
        "/Users/christian/Library/CloudStorage/GoogleDrive-christian.winkelmann@gmail.com/My Drive/Datasets/IguanasFromAbove/Orthomosaics for quality analysis/Snt_STJB01_10012023/output_Snt_STJB01_10012023/corrected_annotations_San_STJB01_10012023.json"
    )
    projected_image = Path(
        "/Users/christian/PycharmProjects/hnee/image_template_search/data/output/matched_template_DJI_0366_Metashape_FCD01-02-03-orthomosaic_cropped.jpg"
    )

    hA_proj = hA_from_file(file_path=projected_annotation_path)
    hA_proj_images = [
        i for i in hA_proj.images if i.image_name in [projected_image.name]
    ]
