...
if __name__ == '__main__':
    ...
    batched_workflow_report = load_yaml_config(yaml_file_path=Path("/Users/christian/PycharmProjects/hnee/image_template_search/data/output/FCD01_02_03/batch_workflow_config_FCD01_02_03.yaml"), cls=BatchWorkflowConfiguration)
    assert isinstance(batched_workflow_report, BatchWorkflowConfiguration), "The loaded config should be a BatchWorkflowConfiguration"
    ...
    try:
        keypoint_class_id = hA.keypoint_schemas[0].keypoint_classes[0].keypoint_class_id # a bit of a hack because there is only one keypoint schema here, but there could be more
    except:
    ...
    for sample in view:
        ...
        if hasattr(sample, "ground_truth_points"):
            for kp in sample.ground_truth_points.keypoints:
                ...
                hkp = Keypoint(x=int(pt.x),
                               y=int(pt.y),
                               norder=0, keypoint_class_id=keypoint_class_id)
                il = ImageLabel(
                    class_name=kp.label,
                    keypoints=[hkp],
                )
                ...
                if hasattr(kp, "hasty_id"):
                    ...
                    if dist > 2:
                        # The object was moved
                        logger.info(f"Object {kp.hasty_id} was moved")
                        il.attributes = il.attributes | image_label.attributes | {"cvat": "moved"}
                        updated_labels.append(il)
                    else:
                        # The object was not moved
                        logger.info(f"Object {kp.hasty_id} was not moved")
                        il.attributes = il.attributes | image_label.attributes | {"cvat": "unchanged"}
        ...
        stats_row["after_correction"] = len(updated_labels) + len(new_labels) + len(unchanged_labels)
